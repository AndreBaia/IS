...
public class PessoaFisicaDAO implements DAO<PessoaFisica, Long> {
	...
	public void insert(PessoaFisica t) {
		try {
			PreparedStatement p = JDBCUtil.getConnection().prepareStatement("insert into tb_pessoa_fisica "
					+ "(nome, endereco, telefone, cpf, email, data_nascimento, sexo) values (?,?,?,?,?,?,?)");
	...
	public void update(PessoaFisica t) {
		try {
			PreparedStatement p = JDBCUtil.getConnection().prepareStatement("update tb_pessoa_fisica "
					+ "set nome = ?, endereco = ?, telefone = ?, cpf = ?, email = ?, data_nascimento = ?, sexo = ? where id = ?");
			p.setString(1, t.getNome());
			p.setString(2, t.getEndereco());
			p.setString(3, t.getTelefone());
			p.setString(4, t.getCpf());
	...
	public List<PessoaFisica> findAll() {
		...
		try {
			...
			while (res.next()) {
				lista.add(
					new PessoaFisica(
							res.getLong("id"), res.getString("nome"), res.getString("endereco"), res.getString("telefone"),
							res.getString("cpf"), res.getString("email"), res.getDate("data_nascimento"),
							res.getString("sexo"))
				);
			}
		} catch (Exception e) {
